(ns com.vadelabs.turbo.css
  (:require-macros [com.vadelabs.turbo.css])
  (:require
   [clojure.spec.alpha :as s]
   [clojure.string :as str]
   [com.vadelabs.turbo.css.impl :as impl]
...
(defn init!
  ...
  [options]
  (let [parsed (s/conform :com.vadelabs.turbo.css.spec/options options)]
    (if (= parsed ::s/invalid)
      (throw (ex-info "Invalid input" (s/explain-data :com.vadelabs.turbo.css.spec/options options)))
...
(defn join
  ...
  ```"
  [& classes]
  (if (s/valid? :com.vadelabs.turbo.css.spec/classes classes)
    (->> classes
         (filter identity)
         (str/join " "))
