(ns com.vadelabs.turbo.hooks
  ...
  #?(:clj (:require [com.vadelabs.turbo.analyzer :as tana])
     :cljs (:require
            ["react" :as react]
            [goog.object :as gobj]))
...
#?(:clj
   (defn deps-macro-body [env deps body deps->hook-body]
     (cond
       ...
       ;; a single symbol
       (and (= (count body) 1) (symbol? (first body)))
       (do (tana/warn tana/warning-simple-body env {:form body})
           nil)
       ...
       (= deps :auto-deps) (deps->hook-body
                             `(cljs.core/array ~@(tana/resolve-local-vars env body))
                             body)
       ...
       (fn
         ([fn-body]
          (vary-meta
            ...
            merge
            {:tag (tana/inferred-type &env fn-body)}))
         ([deps fn-body]
          (vary-meta
            `(raw-use-memo (fn [] ~@fn-body)
                           ~deps)
            merge
            {:tag (tana/inferred-type &env (last fn-body))}))))))
...
#?(:cljs
  ...
  individual callbacks with `use-callback`."
     ...
     (let [[state set-state] (react/useState
       ...
       (com.vadelabs.turbo.hooks/use-effect
